 ...
 component extends="install"{
	...
	function run(){
		...
		args.cfmlEngine = multiSelect()
			...
			.setOptions( [
				{ display : "Lucee 5", value : "lucee@5", selected : true },
				{ display : "Adobe 2016", value : "adobe@2016" },
				{ display : "Adobe 2018", value : "adobe@2018" },
				{ display : "Adobe 2021", value : "adobe@2021" }
			] )
			.ask();

		args.cfmlPassword = ask( "Enter the password for the CFML Engine administrator (Leave empty to use 'contentbox')? " );
		if( !args.cfmlPassword.len() ){
			args.cfmlPassword = "contentbox";
		}

		args.coldboxPassword = ask( "Enter the password for the ColdBox HMVC Reinits (Leave empty to use 'contentbox')? " );
		...
		args.databaseType = multiSelect()
			...
			.setOptions( [
				{ display : "HypersonicSQL", value : "HypersonicSQL" },
				{ display : "MySQL 5.7", value : "MySQL5" },
				{ display : "MySQL 8+", value : "MySQL8", selected : true},
				{ display : "Microsoft SQL Server", value : "MicrosoftSQL" },
				{ display : "PostgreSQL", value : "PostgreSQL" },
		...
		args.databasePassword = ask( "Enter the database password to use for the connection? ")
		if( !args.databasePassword.len() ){
			error( "You must enter a username to continue" );
		}

		args.databaseName = ask( "Enter the database name to use for the connection (Leave empty to use 'contentbox') ? ")
		...
		args.production = multiSelect()
			...
			.setOptions( [
				{ display : "Development", value : "false", selected : true },
				{ display : "Production", value : "true" }
			] )
			.ask();
